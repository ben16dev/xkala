...
extension WorkoutDay {
    ...
    /// Nombre automático para sesiones de roca (ubicación / sector).
    ...
    /// Rocódromo / training: solo nombres automáticos válidos actuales o legacy conocidos.
    private var isTrainingAutoDefaultSessionName: Bool {
        let trimmed = name.trimmingCharacters(in: .whitespacesAndNewlines)
        if trimmed.isEmpty { return true }
        
        // Nombres automáticos actuales
        if trimmed.compare("Rocódromo", options: .caseInsensitive) == .orderedSame {
            return true
        }
        if trimmed.compare("Test en rocódromo", options: .caseInsensitive) == .orderedSame {
            return true
        }
    ...
    /// Roca: bloqueado solo si parece un título manual (p. ej. varias palabras sin ` · `).
    ...
    /// Nombre automático para sesiones de rocódromo (training).
    /// Regla simple: "Test en rocódromo" si hay test completado, "Rocódromo" en caso contrario.
    private var trainingAutoGeneratedName: String {
        hasTestActivity ? "Test en rocódromo" : "Rocódromo"
    }
